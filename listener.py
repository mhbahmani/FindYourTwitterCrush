...
def check_for_new_requests_on_most_liking():
    ...
    for screen_name in repliers:
        if screen_name in handled_users_liking or screen_name in all_progressings: continue
...
def check_for_new_requests_on_likes():
    ...
    for screen_name in repliers:
        if screen_name in handled_users_liked or screen_name in all_progressings: continue
    ...
    direct_requests = client.get_direct_usernames()
    for screen_name in direct_requests:
        if direct_requests[screen_name] in handled_users_liked or : continue
